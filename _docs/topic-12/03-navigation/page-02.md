...
This was helpful up to a point, but how we style these links is very limiting. By changing the **display** property of those links, we can change how each relates to each other as elements.
...
### Block
Just like the display property can turn 'block' elements into `inline` or `inline-block` elements, it can also turn 'inline' elements into `block` elements. This technique is often used to create vertical nav bars out of lists.
...
As you can see, we have used `display: inline;` & `display: inline-block;` to create our first header nav bars! It is often used for that purpose.
